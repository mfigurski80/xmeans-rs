...
pub fn compute_stddev(v: &[f64], free_params: usize) -> f64 {
...
pub fn compute_bic(data: &[&[f64]], model: &KMeansState<f64>) -> f64 {
    let (len, shape) = (data.len() as f64, data[0].len());
    let wrapped_centroids: Vec<&[f64]> = model.centroids.chunks(shape).collect();
    ...
    let errors = assignments
        ...
        .collect::<Vec<f64>>();
    let k = wrapped_centroids.len();
    let free_params = (k - 1) + (shape * k) + 1;
